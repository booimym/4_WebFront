...
function fnTest3() {
    ...
    for(let i = 0; i <arr.length ; i++){
        ...
        arr[i].style.backgroundColor = "rgb(55,"+num+",200)";
    }
}
...
function fnTest4(){
    ...
    alert(input.value); //alert : 알림창 뜨는 거 ㅋ 
...
function fnTest5() {
    ...
    // name 속성 값이 "hobby"인 요소를 모두 얻어와 배열 형식으로 반환하겠다!
    ...
    for(let i = 0 ; i < hobby.length ; i++){
        ...
        // 체크할 수 있는건 오직 radio,checkbox 만 가능해서
        // radio,checkbox 전용 속성이다.
...
function inputEnter(){
    ...
    // 키보드 입력과정
    // - 키 누름 -> keydown 이벤트 -> input태그에 입력
    // -> keypress -> (손을 떼면) keyup

    console.log(window.event.key);
    // keydown : 모든 키가 눌렸을 때
    // keypress : 영어 / 숫자 / 특수문자 / 띄어쓰기가 눌러지고 있을 때
    // -> 꼭 누르고 있을 경우 연속적으로 함수가 호출되는 문제 발생
    // --> keyup을 이럴 때 이용한다(1회만 인식......)
...
function readValue() { //입력된 값을 읽어들여 채팅창에 출력하는 함수
    ...
    //if문을 이용해서 ... 아무 때나 채팅창에 추가하는 게 아니고
    // 채팅이 정상적으로 입력된 경우에만 출력할거임!!!
    ...
    if(input.value.trim().length > 0){
        ...
        //문자열.trim() : 문자열 좌우 공백을 제거(중간 공백 제거는 아님)
