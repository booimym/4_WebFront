...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    // 중간 인덱스 건너뛰면, 건너 뛴 부분은 empty로 남는다.    
    arr1[5] = 111; // 건너뛰어도 추가 가능함!!!
    console.log(arr1);
    ...
    // 2. 배열.forEach(function(item,index){반복 수행 코드})
    // - item : 현재 접근중인 요소
    // - index : 현재 인덱스
    // (이건 배열만!! 쓸 수 있음.. 컬렉션 ㄴㄴ)
...
document.getElementById("btn2").addEventListener("click", function(){
    ...
    // 배열.pop() : 배열의 마지막 요소를 꺼내와서 반환함!!
    const temp = arr.pop(); //마지막 요소를 꺼내와서 temp에 대입!

    console.log("arr : " + arr.toString()); //꺼내지고 3개 요소밖에 안 남음
    ...
    // 결과 : (5) [1, 10, 2, 3, 5] 이렇게 나옴.
    // 이유 : 문자열기준으로 정렬해서 그럼!!

    console.log(numArr.sort(function(a,b){return a-b}));
    //이렇게 하면 숫자 기준 정렬이 된다..... 양수면 자리 바뀌고 그런대 : 1,2,3,5,10

    // * sort() 함수는 원본 배열의 순서를 정렬함
    // -> 원본 훼손이 발생한다....그래서 원본 지키고 싶으면 [깊은 복사] 해야됨...
    console.log(numArr);
...
document.getElementById("btn3").addEventListener("click",function(){
    ...
    lotto.sort(function(a,b){return a-b}); //(2)숫자 정렬
